...
{
    std::ofstream ofs(path.c_str(),std::ios::out|std::ios::trunc);
    for(unsigned int i=0;i<columnNamesCategorized.size()&&columnsCorrespondancesForCategorized[i]<transformedDataColumnsOrdered.size();i++)
    {
    ...
    for(unsigned int i=0;i<N_features;i++)
    {
        for(unsigned int j=0;j<columnNamesCategorized.size()&&columnsCorrespondancesForCategorized[j]<transformedDataColumnsOrdered.size();j++)
        {
            float val = 0;
            if(isCategorized[j])
            {
            ...
            if(j+1<columnNamesCategorized.size()&&columnsCorrespondancesForCategorized[j+1]<transformedDataColumnsOrdered.size())
                ofs<<val<<";";
            else
                ofs<<val;
        }
...
void LoadAndSave<T>::categorize(const std::set<std::string>& toCategorize)
{
    ...
    for(unsigned int i=0;i<chosenColumns.size();i++)
        if(toCategorize.count(chosenColumns[i])) //on crée toutes les colonnes supplémentaires sur le tas
        {
            ...
            std::cout<<"New number of parameters : "<<endIndex<<std::endl;
            curSize = endIndex;
        }
        else
        {
            ...
            curSize++;
            std::cout<<"New number of parameters : "<<curSize<<std::endl;
        }
}
