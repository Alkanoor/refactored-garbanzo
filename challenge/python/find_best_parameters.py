...
#comme scikit learn a ete desinstalle, on utilise le depot clone
#import sys
...
data1 = pandas.read_csv("data/floatCategorizedTest.csv",sep=";")
data2 = pandas.read_csv("data/floatCategorized.csv",sep=";")
...
curN_leaf_min = 4
curDepth = 75
curRatio_filling = 0.26
curOverheap = 7.5

boundsN = (5,25)
boundsN_leaf_min = (2,8)
boundsDepth = (30,100)
boundsRatio_filling = (0.1,0.25,0.02)
boundsOverheap = (1.5,5.0,0.5)

lengthN = boundsN[1]-boundsN[0]
...
n = 0
while n<20:
    print(curN,curN_leaf_min,curDepth,curRatio_filling,curOverheap," iterating on curRatio with bounds ",lengthRatio_filling)
    cur_maxScore = 0
    if curRatio_filling-lengthRatio_filling/2>=0.01:
        x = np.arange(curRatio_filling-lengthRatio_filling/2,curRatio_filling+lengthRatio_filling/2,boundsRatio_filling[2])
    else:
        x = np.arange(0.01,curRatio_filling+lengthRatio_filling/2,boundsRatio_filling[2])
    for i in x:
    ...
    print(curN,curN_leaf_min,curDepth,curRatio_filling,curOverheap," iterating on curOverheap with bounds ",lengthOverheap)
    cur_maxScore = 0
    if curOverheap-lengthOverheap/2>=1.0:
        x = np.arange(curOverheap-lengthOverheap/2,curOverheap+lengthOverheap/2,boundsOverheap[2])
    else:
        x = np.arange(1.0,curOverheap+lengthOverheap/2,boundsOverheap[2])
    for i in x:
    ...
    print(curN,curN_leaf_min,curDepth,curRatio_filling,curOverheap," iterating on N_samples with bounds ",lengthDepth)
    cur_maxScore = 0
    if curDepth-lengthDepth//2>=20:
        x = range(curDepth-lengthDepth//2,curDepth+lengthDepth//2,5)
    else:
        x = range(20,curDepth+lengthDepth//2,5)
    for i in x:
    ...
    print(curN,curN_leaf_min,curDepth,curRatio_filling,curOverheap," iterating on N trees with bounds ",lengthN)
    cur_maxScore = 0
    if curN-lengthN//2>=5:
        x = range(curN-lengthN//2,curN+lengthN//2,1)
    else:
